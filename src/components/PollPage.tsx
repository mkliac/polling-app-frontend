...
const PollForm = () => {
    ...
    useEffect(() => {
        setTotalVotes(poll !== undefined ? poll.items.map((i) => i.voteCount).reduce((a, b) => a + b) : 0);
    }, [poll]);
                ...
                <CardContent sx={{maxWidth:"550px", maxHeight:"80vh", overflow:"auto", scrollbarGutter:"stable"}}>
                    <Box sx={{marginBottom:"12px"}}>
                        {poll.closedDate &&
                            <Typography textAlign="right">
                                Expiry Date: {new Date(poll.closedDate).toLocaleDateString('en-GB')}
                            </Typography>
                        }
                        <Typography textAlign="left" variant="h3" component="div" sx={{wordWrap:"break-word"}}>
                            {poll.title}
                        </Typography>
                        <Typography textAlign="right">
                    ...
                    <Stack overflow="auto" spacing={2} sx={{ flex: 1 }}>
                        {poll.items.map((item, idx) => 
                            <Button key={idx} sx={{textTransform:"none"}} onClick={() => onVote(item)}>
                                <Box sx={{ width:"100%"}}
                                >
                                    ...
                                    <Box sx={{ display:'flex', alignItems:'center'}}>
                                        <Box sx={{ width:'100%', mr:1}}>
                                            <LinearProgress sx={{height:"16px"}} variant="determinate" value={calItemContribution(item)} />
                                        </Box>
                                        <Box sx={{ minWidth: 35 }}>
